...
export default function App() {
  ...
  // Handle magic link callback — reads token from URL hash
  useEffect(() => {
    (async () => {
      ...
      if (at) {
        try {
          ...
          const ns = {
            access_token: at,
            refresh_token: rt,
            expires_at: parseInt(ea || pl.exp),
            user: { id: pl.sub, email: pl.email },
          };
          localStorage.setItem("tb_session", JSON.stringify(ns));
  ...
  const doAddTask = async () => {
    ...
    const t = await sb.createTask(session.access_token, {
  ...
  const doAddNote = async () => {
    ...
    const n = await sb.createNote(session.access_token, {
      task_id: active.id,
      user_id: session.user.id,
      body: noteInput.trim(),
    });
